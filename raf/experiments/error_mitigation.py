...
class ZNEMitigator:
    ...
    def mitigate(
        ...
        mitigated = self._richardson_extrapolate(
            self.scale_factors, expectations
        )
...
class ErrorMitigationExperiment:
    ...
    def run_acceleration_study(
        ...
        print(f"Running Error Mitigation acceleration study...")
        ...
        for iteration in range(num_iterations):
            ...
            for i in range(circuits_per_iteration):
                ...
                result = self.run_single_experiment(
                    circuit, shots=shots, apply_mitigation=True,
            ...
            avg_reduction = np.mean([
                r.error_reduction for r in it_runs 
                if r.error_reduction is not None
            ])
    ...
    def plot_results(self, save_path: Optional[str] = None):
        ...
        for it in df['iteration'].unique():
            ...
            ax.scatter(
                [it] * len(it_data), 
                it_data['noisy_error'],
                alpha=0.5, label='Noisy' if it == 0 else None,
                c='red'
            )
            ax.scatter(
                [it] * len(it_data),
                it_data['mitigated_error'],
                alpha=0.5, label='Mitigated' if it == 0 else None,
                c='blue'
            )
        ...
        if 'iteration_stats' in metrics:
            ...
            ax.axhline(y=metrics.get('overall_acceleration', 1.0) - 1, 
                      color='r', linestyle='--', label=f"Acceleration: {metrics.get('overall_acceleration', 1.0):.2f}")
        ax.set_xlabel('Iteration')
        ax.set_ylabel('Error Reduction')
