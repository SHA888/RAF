...
from raf.core.metrics import CrossLoopCoupling
...
class CrossLoopAnalyzer:
    ...
    def _identify_leverage_points(
        ...
        for loop_name in coupling_matrix:
            ...
            incoming = sum(
                coupling_matrix[other][loop_name] 
                for other in coupling_matrix
            )
            ...
            if loop:
                ...
                avg_addressability = (
                    np.mean([b.addressability_score for b in bottlenecks])
                    if bottlenecks else 0.5
                )
    ...
    def _network_density(self, coupling_matrix: Dict[str, Dict[str, float]]) -> float:
        ...
        actual_connections = sum(
            1 for source in coupling_matrix 
            for target, strength in coupling_matrix[source].items()
            if strength > 0
        )
