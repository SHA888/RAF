...
class LoopDynamicsVisualizer:
    ...
    def plot_coupling_network(
        ...
        for coupling in framework.couplings:
            if coupling.source_loop in G and coupling.target_loop in G:
                G.add_edge(
                    coupling.source_loop,
                    coupling.target_loop,
    ...
    def generate_report(self, framework) -> str:
        ...
        lines.append(f"Active Bottlenecks: {len([b for b in analysis.bottlenecks if b.get('is_active')])}")
        lines.append("")
