...
class ErrorMitigationLoop(AccelerationLoop):
    ...
    def identify_bottlenecks(self) -> List[BottleneckIndicator]:
        ...
        if self.em_state.calibration_cost > 2.0:
            severity = (
                ...
                if self.em_state.calibration_cost > 5.0
        ...
        accuracy_scale_ratio = self.em_state.mitigation_accuracy / (
            self.em_state.experiment_scale / 100
        )
        ...
        if (
            self.em_state.mitigation_accuracy > 0.9
            and accuracy_trend is not None
            and accuracy_trend < 0.01
        ):
            bottlenecks.append(
                BottleneckIndicator(
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if self.em_state.mitigation_accuracy < 0.7:
            recommendations.append("Focus on improving base ML-QEM model architecture and training")
        ...
        if self.em_state.training_data_volume < 10000:
            recommendations.append("Prioritize data collection to enable data-driven improvements")
        ...
        if self.state.status.value == "accelerating":
            recommendations.append("Maintain current trajectory; consider scaling up experiments")
        elif self.state.status.value == "bottlenecked":
            recommendations.append("Address identified bottlenecks before attempting to scale")
