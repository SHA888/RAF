...
class AnsatzDesignLoop(AccelerationLoop):
    ...
    def identify_bottlenecks(self) -> List[BottleneckIndicator]:
        ...
        if self.ad_state.evaluation_cost > 2.0:
            severity = (
                BottleneckSeverity.CRITICAL if self.ad_state.evaluation_cost > 10.0
                else BottleneckSeverity.HIGH if self.ad_state.evaluation_cost > 5.0
        ...
        if self.ad_state.surrogate_accuracy < 0.6:
            severity = (
                BottleneckSeverity.HIGH if self.ad_state.surrogate_accuracy < 0.4
                else BottleneckSeverity.MEDIUM
            )
            bottlenecks.append(BottleneckIndicator(
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if self.ad_state.circuit_quality < 0.7:
            recommendations.append(
                "Expand search to discover higher-quality circuit architectures"
            )
        
        if self.ad_state.problem_coverage < 10:
            recommendations.append(
                "Apply QAS to more problem types to build transferable priors"
            )
        ...
        if self.state.status.value == "accelerating":
            recommendations.append(
                "Loop is accelerating; consider expanding problem coverage"
            )
        elif self.state.status.value == "bottlenecked":
            recommendations.append(
                "Focus on surrogate model improvement to unblock the loop"
            )
